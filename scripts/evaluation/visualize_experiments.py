"""Create presentation-ready figures from existing ThermalSight result files."""
...
def _plot_preprocessing_preview(project_root: Path, path: Path) -> None:
    ...
    fig.suptitle(
        "Thermal Preprocessing Preview", fontsize=22, fontweight="bold", color=TEXT, y=0.995
    )
    fig.text(
        0.5, 0.958,
        "Identical night-thermal frames after Baseline, CLAHE, and Bilateral + CLAHE processing",
...
def _plot_metric_delta(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    for offset, experiment in ((-height / 2, "clahe"), (height / 2, "bilateral_clahe")):
        ...
        bars = axis.barh(
            ...
            label=f"{DISPLAY_NAMES[experiment]} vs Baseline", alpha=0.92,
        )
    ...
    axis.set_title("Model Performance Comparison Against Baseline", loc="left", fontsize=18, fontweight="bold", color=TEXT, pad=18)
    axis.text(
        0, 1.01, "Positive values indicate improvement; negative values indicate a trade-off.",
...
def _plot_recall_curve(path: Path, curves: dict[str, list[float]]) -> None:
    ...
    axis.set_title("Validation Recall throughout Training", loc="left", fontsize=18, fontweight="bold", color=TEXT, pad=18)
    axis.text(
        0, 1.01, "Raw epoch-level values from Ultralytics results.csv (no smoothing)",
        transform=axis.transAxes, fontsize=10, color="#64748B", va="bottom",
    )
...
def _plot_recall_latency(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    axis.set_title("Recall–Latency Trade-off", loc="left", fontsize=19, fontweight="bold", color=TEXT, pad=18)
    axis.text(
        0, 1.01, "CLAHE delivers the highest Recall with a limited preprocessing cost increase.",
        transform=axis.transAxes, fontsize=10, color="#64748B", va="bottom",
    )
    axis.text(
        0.02, 0.95, "Preferred direction  ↖", transform=axis.transAxes,
...
def _plot_confusion_matrices(project_root: Path, path: Path) -> None:
    ...
    fig.suptitle(
        "Confusion Matrices by Preprocessing Model",
