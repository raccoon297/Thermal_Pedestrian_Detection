...
from scripts.preprocessing.thermal import preprocess_clahe_optimized

DEMO_CONF = FINAL_CONFIDENCE
MATCH_IOU = 0.50
DEMO_SEED = 23
...
def _model_path(project_root: Path) -> Path:
    current_path = final_weights_path(project_root)
...
def select_validation_samples(project_root: Path, seed: int = DEMO_SEED) -> list[Path]:
    """Select four deterministic positive raw thermal validation frames."""
    ...
    thermal_dir = project_root / "data" / "processed" / "thermal_night" / "source_tiff" / "val"
    ...
    selected = random.Random(seed).sample(candidates, 4)
    thermal_paths = [thermal_dir / f"{path.stem}.tiff" for path in selected]
    missing = [path for path in thermal_paths if not path.is_file()]
    if missing:
        raise FileNotFoundError(f"Raw thermal validation frame is missing: {missing[0]}")
...
def _sample_card(
    sample: SampleResult, sample_number: int, width: int = 640, image_height: int = 512,
) -> np.ndarray:
    ...
    cv2.putText(
        card, f"Validation sample {sample_number}", (14, 29),
        cv2.FONT_HERSHEY_SIMPLEX, 0.51, WHITE, 1, cv2.LINE_AA,
    )
    stats = (
...
def draw_validation_demo(samples: list[SampleResult], output_path: Path) -> None:
    ...
    _put_centered(
        canvas, "FOUR VALIDATION EXAMPLES", canvas_width // 2, 39, 0.82, WHITE, 2,
    )
    _put_centered(
        canvas,
        "Optimized CLAHE + YOLO26n (960)  |  Ground truth: dashed cyan  |  Prediction: solid orange",
    ...
    for index, sample in enumerate(samples):
        ...
        canvas[y:y + card_height, x:x + card_width] = _sample_card(sample, index + 1)
    ...
    summary = (
        f"Four-image summary   |   Recall {recall * 100:.1f}%   |   GT persons {total_gt}   |   "
...
def run_image_demo(project_root: Path, model_bundle: tuple[Any, Any, Path] | None = None) -> dict[str, Any]:
    ...
    for path in selected:
        ...
        if thermal.ndim == 3 and thermal.shape[2] == 1:
            thermal = thermal[:, :, 0]
        processed = preprocess_clahe_optimized(thermal)
        rendered = cv2.cvtColor(processed, cv2.COLOR_GRAY2BGR)
        model_inputs.append(rendered)
        rendered_inputs.append(rendered)
    predictions = model.predict(
        source=model_inputs, imgsz=IMAGE_SIZE, conf=DEMO_CONF,
        classes=[FINAL_PERSON_CLASS_ID], device=device, verbose=False, save=False,
    )
    label_dir = project_root / "data" / "processed" / "thermal_night" / "model_input" / "clahe" / "labels" / "val"
    samples = []
    for path, image_bgr, prediction_result in zip(selected, rendered_inputs, predictions):
        gt = load_yolo_labels(
            label_dir / f"{path.stem}.txt", image_bgr.shape[1], image_bgr.shape[0]
        )
...
def _compose_video_frame(left: np.ndarray, right: np.ndarray) -> np.ndarray:
    ...
    _put_centered(
        canvas, "OPTIMIZED CLAHE + YOLO26n (960)",
        width + width // 2, 43, 0.55, ACCENT, 1,
    )
...
def create_gif_preview(video_path: Path, output_path: Path) -> dict[str, Any]:
    """Create the compact animated preview embedded in README.md."""
    ...
    try:
        while True:
            ...
            if frame_index % GIF_FRAME_STEP == 0:
                ...
                resized = cv2.resize(
                    frame, (GIF_WIDTH, gif_height), interpolation=cv2.INTER_AREA
                )
    ...
    with Image.open(output_path) as image:
        saved_frames = getattr(image, "n_frames", 1)
        if saved_frames != len(frames) or image.size != frames[0].size:
            raise RuntimeError("Generated GIF verification failed")
    return {
        "output": output_path,
        "frames": len(frames),
        "width": frames[0].width,
        "height": frames[0].height,
        "duration_seconds": len(frames) * GIF_DURATION_MS / 1000.0,
    }
...
def run_video_demo(project_root: Path, model_bundle: tuple[Any, Any, Path] | None = None) -> dict[str, Any]:
    ...
    gif_details = create_gif_preview(
        output_path, project_root / "results" / "demo" / "demo.gif"
    )
    details["gif"] = gif_details
    print(f"\nSaved:\n{output_path}\n{gif_details['output']}")
    return details
